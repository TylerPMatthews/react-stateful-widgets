...
export default function Programmers() {
 ...
 const [featuredProgrammer, setFeaturedProgrammer] = useState(null)
  ...
  const getNameOfFeatured = () => {
   ...
   setFeaturedProgrammer(event.dev.id)
  };
      ...
      <div className='programmers'>
        {
          ...
          programmersList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={(event) => { /* in here set the featured id to be dev.id */ featuredProgrammer(event.dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
